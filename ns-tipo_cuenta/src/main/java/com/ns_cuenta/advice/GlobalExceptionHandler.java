package com.ns_cuenta.advice;

//Esta clase es un controlador global de excepciones

import com.ns_cuenta.erros.ApiErrors;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    //Para generar el codigo: Click derecho, generate, overridemetods
    ...
    //con los datos necesarios(mensajes, code status hhtp, timestamp)
    ...
    protected ResponseEntity<Object> handleHttpMediaTypeNotSupported(HttpMediaTypeNotSupportedException ex, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        List<String> detalles = new ArrayList<String>();
        detalles.add("MediaType no soportado por el método");
    ...
    protected ResponseEntity<Object> handleMissingServletRequestParameter(MissingServletRequestParameterException ex, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        List<String> detalles = new ArrayList<String>();
        detalles.add("Parametro de peticion no encontrado");
        ApiErrors errores = new ApiErrors(ex.getMessage(), detalles, status, LocalDateTime.now());
        return ResponseEntity.status(status).body(errores);
    }
    ...
    //Con @ExceptionHandler puedo agregar mis propias excepciones personalizadas al manejador global
